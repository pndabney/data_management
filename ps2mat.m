function varargout=ps2mat(file)
% [t,d]=PS2MAT(file)
%
% Reads, and converts CSV file from InSight's PS data to a MATLAB file
...
if exist(fullfile(path,matname))~=2
    ...
    % Number of PS variables - 9
    N = 9;
    fmt=['%.3f%.3f%s%s%s',repmat('%.4f',[1, N-5])];
    % Read in the 'data'
