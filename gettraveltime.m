function varargout=gettraveltime(direc, model,phases,staloc,evloc,edepth,ofile)
% [tt_query]=gettraveltime(direc,model,phases,staloc,evloc,edepth,ofile)
%
...
% edepth        Event depth
% ofile         Optional output file name
%
% OUTPUT:
%
...
switch nargin
  case 7
    filename = srintf('%s.txt',ofile);
  case 6
    tdate = string(datetime('today'),'yyyy.MM.dd');
    filename = sprintf('TRAVELTIMES.%s.txt',tdate);
end
